...
export async function runRatingsSeeder(
  ds: DataSource,
  users: User[],
  listings: Listing[],
) {
  ...
  if (!seller || !buyerOne || !buyerTwo) {
    throw new Error(
      'Seeding ratings requires Gia, Rumnaz, and Kriti test users.',
    );
  }

  const officeChairListing = listings.find(
    (l) => l.title === 'Office Chair — ergonomic',
  );
  const monitorListing = listings.find(
    (l) => l.title === '27" Dual Monitor Bundle',
  );

  if (
    !officeChairListing ||
    officeChairListing.sold_to_user_id !== buyerOne.user_id
  ) {
    throw new Error(
      'Office Chair listing must be sold to Rumnaz before rating seed can run.',
    );
  }

  if (!monitorListing || monitorListing.sold_to_user_id !== buyerTwo.user_id) {
    throw new Error(
      'Monitor bundle listing must be sold to Kriti before rating seed can run.',
    );
  }
  ...
  const ratingDefinitions = [
    ...
    },
    {
      ...
      rating: 4,
      review:
        'Great deal overall. Packaging could have been better but still happy!',
    },
  ];
