...
const frontendOrigin =
  process.env.E2E_FRONTEND_ORIGIN || `http://${defaultHost}:${frontendPort}`;
...
module.exports = defineConfig({
  ...
  // 👇 NEW: store Playwright artifacts here (traces, videos, screenshots)
  // This matches the artifact upload path `frontend/test-results`
  outputDir: path.join(__dirname, 'test-results'),

  use: {
    baseURL: process.env.E2E_BASE_URL || `${frontendOrigin}`,

    // 👇 Trace/video/screenshot configuration
    // - trace: keep a full trace only when a test fails (cleaner for CI)
    // - video: keep video only for failing tests
    // - screenshot: only capture on failure
    trace: 'retain-on-failure',      // was: 'on-first-retry'
    video: 'retain-on-failure',      // already had this
    ...
    ? [
        ...
        },
        {
          ...
          env: {
            ...
            SESSION_SECRET:
              process.env.SESSION_SECRET || 'playwright-test-secret',
          },
        },
      ]
    : [],
